...
// Dummy API Key list (Replace with secure storage or env variables)
global.apikey = ["jundev18"];
...
async function createQRIS(amount, codeqr) {
    try {
        ...
        const finalString = step2[0] + uang + step2[1];
        const crc = convertCRC16(finalString);
        const result = finalString + crc;
...
module.exports = function (app) {
    app.get('/orderkuota/createpayment', async (req, res) => {
        ...
        if (!global.apikey.includes(apikey)) {
            return res.status(403).json({ message: "Apikey tidak valid." });
        }
